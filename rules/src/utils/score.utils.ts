...
import { Astrid, DwergAesir, DwergBergelmir, DwergJungir, DwergSigmir, DwergYmir, Heroes, Idunn } from '../cards/Heroes'
...
export const getWarriorScore = (game: GameState | GameView, playerId: PlayerId) => {
  ...
  if (majorityPlayers.includes(playerId) && game.step === Step.Scoring) {
...
export const getMaximumCoinValue = (game: GameState | GameView, playerId: PlayerId) => {
  ...
  if (playerCoins.some((c) => c.id === undefined)) {
    return 0
  }
...
export const getNeutralScoring = (game: GameState | GameView, playerId: PlayerId) => {
...
export const getPlayerScore = (game: GameState | GameView, playerId: PlayerId) => {
  return (
    ...
    getNeutralScoring(game, playerId)
  )
}
