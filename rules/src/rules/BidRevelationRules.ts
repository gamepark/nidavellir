...
class BidRevelationRules extends MaterialRulesPart {
  ...
  afterItemMove(move: ItemMove) {
    if (isMoveItemType(MaterialType.Coin)(move)) {
      ...
      const playerWithUline = this.material(MaterialType.Card)
        .location(LocationType.CommandZone)
        .id((id: CardId) => id.front === Card.Uline)
        .getItem()

      if (playerWithUline) {
        const tavernCoin = this.material(MaterialType.Coin)
  ...
  get moveToElvalandTurn(): MaterialMove[] {
    return [this.startPlayerTurn(RuleId.ElvalandTurn, new TurnOrder(this.game).nextPlayer)]
  }

  get coinsToReveal() {
    return this.material(MaterialType.Coin).location(
      (location) => location.type === LocationType.PlayerBoard && location.id === this.tavern && !location.rotation
    )
  }
}
