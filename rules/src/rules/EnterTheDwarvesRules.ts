...
class EnterTheDwarvesRules extends MaterialRulesPart {
  onRuleStart(): MaterialMove[] {
    ...
    for (const player of this.game.players) {
      ...
      moves.push(...coins.location((location) => LocationType.Hand !== location.type).moveItems({ type: LocationType.Hand, player }), coins.shuffle())
    }
  ...
  get playersThatMustBid() {
    return this.game.players.filter((player) => {
      const uline = this.material(MaterialType.Card)
