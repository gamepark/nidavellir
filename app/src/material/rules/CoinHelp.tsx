  ...
  return (
    <>
      ...
      {visible && (
        <p>
          <Trans
            i18nKey="rule.coin.description"
            defaults="It's a {color} coin with a value of {value}."
            values={{ value: Coins[item.id as Coin].value, color: t(`rule.coin.color.${Coins[item.id as Coin].color}`) }}
          >
            <strong />
          </Trans>
        </p>
...
const PlaceCoinMoves: FC<MaterialHelpProps> = (props) => {
  ...
  const placeCoins = useLegalMoves(
    (move) =>
      isMoveItemType(MaterialType.Coin)(move) &&
      move.itemIndex === itemIndex &&
      (move.location.type === LocationType.PlayerBoard || move.location.type === LocationType.Hand)
  )
  ...
  return (
    <>
      ...
      <div css={buttonContainer(placeCoins.length === 1 ? 1 : 2)}>
        {placeCoins.map((move) => (
          <PlaceCoinButton key={JSON.stringify(move)} move={move} {...props} />
        ))}
      </div>
    </>
  )
  ...
  return (
    <>
      ...
      <div css={buttonContainer(tradeCoins.length === 1 ? 1 : 2)}>
        {tradeCoins.map((move) => (
          <TradeCoinButton key={JSON.stringify(move)} move={move} {...props} />
        ))}
      </div>
    </>
  )
...
const TransformCoinsMoves: FC<MaterialHelpProps> = (props) => {
  ...
  const transformCoins = useLegalMoves(
    (move) =>
  ...
  return (
    <>
      ...
      <div css={buttonContainer(transformCoins.length === 1 ? 1 : 2)}>
        {transformCoins.map((move) => (
          <TransformCoinButton key={JSON.stringify(move)} move={move} {...props} />
        ))}
      </div>
    </>
  )
